<x-app-layout>
    @section('subtitle', 'Изменение настроек терминала')
    @section('title', 'Изменить терминал Hikvision')
        ...
        <div class="bg-white rounded-lg shadow border border-gray-200 p-6">
            <form method="POST" action="{{ route('hikvision.update', $hikvision) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <x-text-input id="password" name="password" type="password" class="mt-1 block w-full" placeholder="Оставьте пустым, чтобы не менять" />
                    <p class="mt-1 text-xs text-gray-400">Оставьте пустым, чтобы сохранить текущий пароль.</p>
                    <x-input-error :messages="$errors->get('password')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="location" value="Местоположение" />
                    <x-text-input id="location" name="location" type="text" class="mt-1 block w-full" :value="old('location', $hikvision->location)" placeholder="Главный вход" />
                ...
                <div>
                    ...
                    <select id="direction" name="direction" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        ...
                        <option value="in" @selected(old('direction', $hikvision->direction) === 'in')>Вход</option>
                        <option value="out" @selected(old('direction', $hikvision->direction) === 'out')>Выход</option>
                    </select>
