<x-app-layout>
    ...
    <div class="max-w-xl">
        <div class="bg-white rounded-lg shadow border border-gray-200 p-6">
            <form method="POST" action="{{ route('access-points.store') }}" class="space-y-4"
                x-data="{
                    accessPoints: @js($accessPoints),
                    selectedId: '{{ old('rusguard_access_point_id') }}',
                    get selectedName() {
            ...
            >
                ...
                <div>
                    ...
                    <select id="enter_device_id" name="enter_device_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        ...
                        @foreach($devices as $device)
                            <option value="{{ $device->id }}" @selected(old('enter_device_id') == $device->id)>{{ $device->name }} ({{ $device->sn }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="exit_device_id" name="exit_device_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        <option value="">— None —</option>
                        @foreach($devices as $device)
                            <option value="{{ $device->id }}" @selected(old('exit_device_id') == $device->id)>{{ $device->name }} ({{ $device->sn }})</option>
                        @endforeach
                    </select>
