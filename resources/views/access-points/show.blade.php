<x-app-layout>
    @section('subtitle', 'Информация о точке доступа')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-lg shadow border border-gray-200 p-5">
            <h2 class="text-sm font-semibold text-gray-700 mb-3">Детали</h2>
            <dl class="text-sm divide-y divide-gray-50">
                <div class="py-2"><dt class="text-xs font-medium text-gray-400 uppercase tracking-wide">Точка доступа</dt><dd class="text-gray-700 mt-0.5">{{ $accessPoint->rusguard_access_point_name }}</dd></div>
                ...
                <div class="py-2 flex justify-between gap-4 items-center"><dt class="text-xs font-medium text-gray-400 uppercase tracking-wide">Статус</dt><dd>
                    ...
                    @else
                        <span class="inline-flex px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-500 rounded-full">Неактивна</span>
                    @endif
                </dd></div>
            </dl>
        ...
        {{-- Employees --}}
        <div class="bg-white rounded-lg shadow border border-gray-200 p-5">
            <h2 class="text-sm font-semibold text-gray-700 mb-3">Сотрудники ({{ $accessPoint->employees->count() }})</h2>
            <div class="space-y-1 max-h-80 overflow-y-auto">
                ...
                @empty
                    <p class="text-sm text-gray-400">Сотрудники ещё не синхронизированы.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow border border-gray-200 p-5">
            ...
            <div class="space-y-1 max-h-80 overflow-y-auto">
                @forelse($recentEvents as $event)
                    <div class="py-1.5 text-sm border-b border-gray-50 last:border-0">
                        <div class="flex items-center justify-between">
                            <span class="text-gray-800 font-medium">{{ $event->employee?->full_name ?? 'Неизвестно' }}</span>
                ...
                @empty
                    <p class="text-sm text-gray-400">Пока нет событий.</p>
                @endforelse
            </div>
        </div>
    </div>
