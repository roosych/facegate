...
class MonitorControllerTest extends TestCase
{
    ...
    public function test_show_rejects_an_unsigned_url(): void
    {
        $accessPoint = AccessPoint::factory()->create();

        $response = $this->get("/monitor/{$accessPoint->id}");

        $response->assertForbidden();
    }

    public function test_show_renders_for_a_signed_url(): void
    {
        $accessPoint = AccessPoint::factory()->create();

        $response = $this->get(URL::signedRoute('monitor.show', ['accessPoints' => (string) $accessPoint->id]));

        $response->assertOk();
    }

    public function test_show_renders_several_access_points_on_one_screen(): void
    {
        $first = AccessPoint::factory()->create(['name' => 'Turnstile A']);
        $second = AccessPoint::factory()->create(['name' => 'Turnstile B']);

        $response = $this->get(URL::signedRoute('monitor.show', ['accessPoints' => "{$first->id},{$second->id}"]));

        $response->assertOk();
        $response->assertSee('Turnstile A');
        $response->assertSee('Turnstile B');
    }

    public function test_show_404s_when_none_of_the_ids_exist(): void
    {
        $response = $this->get(URL::signedRoute('monitor.show', ['accessPoints' => '999999']));

        $response->assertNotFound();
    }

    public function test_status_merges_a_turnstiles_terminals_into_one_block(): void
    {
        $accessPoint = AccessPoint::factory()->create();
        ...
        $response = $this->get(URL::signedRoute('monitor.status', ['accessPoints' => (string) $accessPoint->id]));
    ...
    public function test_status_reports_one_entry_per_access_point_in_the_given_order(): void
    {
        $first = AccessPoint::factory()->create();
        $second = AccessPoint::factory()->create();
        HikvisionTerminal::factory()->create(['access_point_id' => $first->id]);
        HikvisionTerminal::factory()->create(['access_point_id' => $second->id]);

        $response = $this->get(URL::signedRoute('monitor.status', ['accessPoints' => "{$second->id},{$first->id}"]));

        $response->assertOk();
        $response->assertJsonCount(2, 'access_points');
        $response->assertJsonPath('access_points.0.access_point_id', $second->id);
        $response->assertJsonPath('access_points.1.access_point_id', $first->id);
    }

    public function test_status_reports_no_event_when_nothing_is_cached_yet(): void
    {
        $accessPoint = AccessPoint::factory()->create();
